...
&nbsp;
  ...
  <div class="title">
   ...
   <form name="information" method="Post" action="payadd.jsp">
  <div class="main">
    <div class="content">
      <div class="first">
        <div class="firstin">
          <div class="img1">
<%
String  honda[] = request.getParameterValues("honda_Id");
        int y=0;
        int x=honda.length;

        String idda="";
       String hondaString[]=request.getParameterValues("honda_Id");
	         double tota=0;
        int g=honda.length;
        int left;
        String idd;
       String leftString;
        ...
        while(rs.next())
        {  
  		  if(honda[y].equals(rs.getString(1)))
           {     
            hondaString[y] =rs.getString(2);
            tota=tota+rs.getInt(6)*0.8;
			...
			idda = rs.getString(1);
            idd = rs.getString(1);
            sql="update honda set stock='"+leftString+"'WHERE honda_id="+idd;
           con.createStatement().execute(sql);
         ...
         out.println("<h1>"+hondaString[y]+"</h1>");
		 session.setAttribute("hondaname",hondaString[y]);
		 session.setAttribute("hondaph",idda);
		           session.setAttribute("total",tota);
		 %>
		 <img class="payimg" src="home/H<%=session.getAttribute("hondaph")%>.png">
		 <%
...
%>
          ...
          <div class="paytxt">
		             <div>
              <h2>姓名*:</h2><input name="buyer" class="name" type="text" required="required">
            </div>
            <div class="sex">
              <h2>性別*:</h2>
			  
         		<input name="sex" value="man" type="radio"required="required">
								<h2>男</h2>
				<input name="sex" value="lady" type="radio"required="required">
				<h2>女</h2>
			
			
            </div>
            <div>
              <h2>身分證字號*:</h2><input name="code" class="code" type="text"required="required">
            </div>
            <div class="payway">
              <h2>付款方式*:</h2>
		
              <input name="pway" value="bTwo" type="radio"required="required">
              <h2>現金</h2>
              <input name="pway" value="bOne" type="radio"required="required">
              <h2>信用卡</h2>
            </div>
