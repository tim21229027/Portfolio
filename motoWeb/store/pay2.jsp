...
&nbsp;
  ...
  <div class="title">
    ...
    <form name="information" method="Post" action="payadd2.jsp">
  <div class="main">
    <div class="content">
      <div class="first">
        <div class="firstin">
          <div class="img1">
<%
String  yamaha[] = request.getParameterValues("yamaha_Id");
        int y=0;
        int x=yamaha.length;
        String iddb="";
       String yamahaString[]=request.getParameterValues("yamaha_Id");
	         double tota2=0;
        int g=yamaha.length;
        int left;
        String idd;
       String leftString;
        ...
        while(rs.next())
        {  
  		  if(yamaha[y].equals(rs.getString(1)))
           {     
            yamahaString[y] =rs.getString(2);
            tota2=tota2+rs.getInt(6)*0.8;
			...
			iddb = rs.getString(1);
            idd = rs.getString(1);
            sql="update yamaha set stock='"+leftString+"'WHERE yamaha_id="+idd;
           con.createStatement().execute(sql);
         ...
         out.println("<h1>"+yamahaString[y]+"</h1>");
		 session.setAttribute("yamahaname",yamahaString[y]);
		 session.setAttribute("yamahaph",iddb);
		           session.setAttribute("total2",tota2);
		 %>
		 <img class="payimg" src="home/y<%=session.getAttribute("yamahaph")%>.png">
		 <%
...
%>
          ...
          <div class="paytxt">
		             <div>
              <h2>姓名*:</h2><input name="buyer2" class="name" type="text" required="required">
            </div>
            <div class="sex">
              <h2>性別*:</h2>
			  
         		<input name="sex2" value="man" type="radio" required="required">
								<h2>男</h2>
				<input name="sex2" value="lady" type="radio" required="required">
				<h2>女</h2>
			
			
            </div>
            <div>
              <h2>身分證字號*:</h2><input name="code2" class="code" type="text" required="required">
            </div>
            <div class="payway">
              <h2>付款方式*:</h2>
		
              <input name="pway2" value="bTwo" type="radio" required="required">
              <h2>現金</h2>
              <input name="pway2" value="bOne" type="radio" required="required">
              <h2>信用卡</h2>
            </div>
